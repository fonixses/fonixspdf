  ...
  return <div className={`converter-panel ${compact ? "p-4 sm:p-6" : "p-5 sm:p-8"}`}>
    {!files.length ? <FileUploader accept={accepted} multiple={tool?.multiple} onFiles={addFiles} /> : <>
      ...
      <div className="mt-6 grid gap-4 border-t pt-6 sm:grid-cols-2">
        ...
        {tool?.compression && <Field label="Compression"><select value={compression} onChange={(event) => setCompression(event.target.value)}><option value="low">Lossless structure optimization</option><option value="recommended">Balanced image optimization</option><option value="high">Strong image optimization</option></select></Field>}
